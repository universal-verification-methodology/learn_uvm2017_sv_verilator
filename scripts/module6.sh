...
run_example() {
  ...
  # Find DUT files in module6/dut directory
  local dut_files=()
  if [[ -d "$MODULE6_DIR/dut" ]]; then
    while IFS= read -r -d '' file; do
      dut_files+=("$file")
    done < <(find "$MODULE6_DIR/dut" -type f \( -name "*.v" -o -name "*.sv" \) -print0 2>/dev/null || true)
  fi
  ...
  verilator -sv --cc --exe --timing --trace --binary \
    ...
    "$uvm_src_dir/uvm_pkg.sv" "${dut_files[@]}" "$sv" "$cpp" --top-module "$top_module" 2>&1 | tee compile.log
