...
OPTIONS:
    UVM Examples:
        ...
        --coverage           Run coverage examples
        --configuration      Run configuration examples
...
check_prerequisites() {
    ...
    # Check UVM library (required for Module 5)
    local uvm_pkg_file=""
...
run_uvm_tests() {
    ...
    cd "$MODULE5_DIR/tests/uvm_tests" || {
        print_status $RED "Error: Failed to change to UVM tests directory"
        return 1
    }
    
    print_status $BLUE "Running UVM advanced test..."
    ...
    make -j"$PARALLEL_JOBS" SIM="$SIMULATOR" TEST=test_advanced_uvm 2>&1 | tee /tmp/uvm_advanced.log
    local exit_code=${PIPESTATUS[0]}
    set -e  # Re-enable exit on error
    if [[ $exit_code -eq 0 ]]; then
        print_status $GREEN "✓ UVM advanced test passed"
        cd "$PROJECT_ROOT"
        return 0
    else
        print_status $RED "✗ UVM advanced test failed (exit code: $exit_code)"
        print_status $YELLOW "Check /tmp/uvm_advanced.log for details"
        if [[ "$SIMULATOR" == "verilator" ]]; then
            print_status $YELLOW ""
            ...
            print_status $YELLOW "      cd module5/tests/uvm_tests"
            print_status $YELLOW "      make SIM=vcs TEST=test_advanced_uvm      # For VCS"
            print_status $YELLOW "      make SIM=questa TEST=test_advanced_uvm    # For QuestaSim"
            print_status $YELLOW "      make SIM=xcelium TEST=test_advanced_uvm   # For Xcelium"
        else
    ...
    if [[ "$RUN_VIRTUAL_SEQUENCES" == true ]] || [[ "$RUN_COVERAGE" == true ]] || \
       ...
       [[ "$RUN_REGISTER_MODEL" == true ]]; then
        ...
        if [[ "$RUN_VIRTUAL_SEQUENCES" == true ]]; then
            if ! run_uvm_example "virtual_sequences" "VirtualSequences" "virtual_sequences.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_COVERAGE" == true ]]; then
            if ! run_uvm_example "coverage" "Coverage" "coverage.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_CONFIGURATION" == true ]]; then
            if ! run_uvm_example "configuration" "Configuration" "configuration.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_CALLBACKS" == true ]]; then
            if ! run_uvm_example "callbacks" "Callbacks" "callbacks.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_REGISTER_MODEL" == true ]]; then
            if ! run_uvm_example "register_model" "RegisterModel" "register_model.sv"; then
                errors=$((errors + 1))
            fi
        fi
    fi
