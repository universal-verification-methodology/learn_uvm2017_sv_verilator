...
check_prerequisites() {
    ...
    # Check UVM library (required for Module 4)
    local uvm_pkg_file=""
...
run_uvm_tests() {
    ...
    cd "$MODULE4_DIR/tests/uvm_tests" || {
        print_status $RED "Error: Failed to change to UVM tests directory"
        return 1
    }
    
    print_status $BLUE "Running UVM complete agent test..."
    ...
    make -j"$PARALLEL_JOBS" SIM="$SIMULATOR" TEST=test_complete_agent_uvm 2>&1 | tee /tmp/uvm_complete_agent.log
    local exit_code=${PIPESTATUS[0]}
    set -e  # Re-enable exit on error
    if [[ $exit_code -eq 0 ]]; then
        print_status $GREEN "✓ UVM complete agent test passed"
        cd "$PROJECT_ROOT"
        return 0
    else
        print_status $RED "✗ UVM complete agent test failed (exit code: $exit_code)"
        print_status $YELLOW "Check /tmp/uvm_complete_agent.log for details"
        if [[ "$SIMULATOR" == "verilator" ]]; then
            print_status $YELLOW ""
            ...
            print_status $YELLOW "      cd module4/tests/uvm_tests"
            print_status $YELLOW "      make SIM=vcs TEST=test_complete_agent_uvm      # For VCS"
            print_status $YELLOW "      make SIM=questa TEST=test_complete_agent_uvm    # For QuestaSim"
            print_status $YELLOW "      make SIM=xcelium TEST=test_complete_agent_uvm   # For Xcelium"
        else
...
main() {
    ...
    # Run UVM examples
    if [[ "$RUN_TRANSACTIONS" == true ]] || [[ "$RUN_DRIVERS" == true ]] || \
       [[ "$RUN_MONITORS" == true ]] || [[ "$RUN_SEQUENCERS" == true ]] || \
       [[ "$RUN_TLM" == true ]] || [[ "$RUN_SCOREBOARDS" == true ]] || \
       [[ "$RUN_AGENTS" == true ]]; then
        
        print_header "Running UVM Examples"
        
        if [[ "$RUN_TRANSACTIONS" == true ]]; then
            if ! run_uvm_example "transactions" "Transactions" "transactions.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_DRIVERS" == true ]]; then
            if ! run_uvm_example "drivers" "Drivers" "drivers.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_MONITORS" == true ]]; then
            if ! run_uvm_example "monitors" "Monitors" "monitors.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_SEQUENCERS" == true ]]; then
            if ! run_uvm_example "sequencers" "Sequencers" "sequencers.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_TLM" == true ]]; then
            if ! run_uvm_example "tlm" "TLM" "tlm.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_SCOREBOARDS" == true ]]; then
            if ! run_uvm_example "scoreboards" "Scoreboards" "scoreboards.sv"; then
                errors=$((errors + 1))
            fi
        fi
        
        if [[ "$RUN_AGENTS" == true ]]; then
            if ! run_uvm_example "agents" "Agents" "agents.sv"; then
                errors=$((errors + 1))
            fi
        fi
    fi
    ...
    if [[ $errors -eq 0 ]]; then
        ...
        echo "  3. Proceed to Module 5: Advanced UVM"
        echo ""
