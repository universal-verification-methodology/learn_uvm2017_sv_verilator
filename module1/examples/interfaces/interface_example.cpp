/**
 * Module 1 Example 1.2: Interface Example - C++ Testbench
 * 
 * This C++ file serves as the testbench wrapper for the SystemVerilog
 * interface example. It instantiates the Verilator-generated model,
 ...
 * APB bus transactions which require multiple clock cycles to complete.
 */
...
// The class name is 'Vinterface_example' (V prefix + module name)
 ...
 * 4. Run the simulation loop (with longer timeout for bus transactions)
...
int main(int argc, char** argv) {
    ...
    // The module was compiled by Verilator from interface_example.sv
    // This instance represents the entire SystemVerilog design including interfaces
    ...
    // This is especially useful for viewing bus transactions on interfaces
    ...
    // Depth 99 traces all levels of the design hierarchy, including interface signals
    top->trace(tfp, 99);
    
    // Open the VCD file for writing
    // "interface_example.vcd" is the output filename
    // This file will contain all signal transitions during simulation
    // Interface signals will be visible in the waveform viewer
    ...
    // Interface signals are also initialized at this point
    top->eval();
    
    // Dump initial state to VCD file at time 0
    // This captures the initial values of all traced signals
    // Interface signals start in their default state
    tfp->dump(main_time);
    
    // Main simulation loop: run until finish or timeout
    // 'Verilated::gotFinish()' returns true when SystemVerilog $finish is called
    // 'main_time < 10000' provides longer timeout for bus transaction demonstrations
    // The longer timeout allows APB read/write cycles to complete
    while (!Verilated::gotFinish() && main_time < 10000) {
        ...
        // Clock edges and interface signal changes occur at these time steps
        main_time++;
        ...
        // Interface signals are updated as part of this evaluation
        top->eval();
        
        // Dump current signal values to VCD file
        // This records the state of all traced signals at this time point
        // Interface signal transitions are captured for bus protocol analysis
        tfp->dump(main_time);
    }
    ...
    // The file can now be opened in waveform viewers to analyze bus transactions
    tfp->close();
